...
- `make verify` target — canonical one-command verification entrypoint.

### Fixed
- Resolved 10 `ruff` lint warnings in `tests/test_bot.py` (E402, F841, E741).
- `TRADING_PAIRS` config parsing now trims whitespace and drops empty items from environment input.
